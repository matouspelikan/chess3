...
//            gameTimer = new ChessTimer(timerLabelUp, timerLabelDown,this, gameData.timer1, gameData.timer2, this.game.upColor == Color.WHITE, false);
            ...
            if(gameData.editBeforeStart)
                this.game.gameBoard.clearHistories();
...
//            gameTimer = new ChessTimer(timerLabelUp, timerLabelDown,this, defaultMoveTime, defaultMoveTime, this.game.upColor == Color.WHITE, false);
    ...
    public void SquareClicked(Position clickedPosition){
        ...
        if(editing && creatingNewPiece){
            ...
            if(this.game.gameBoard.getBoard()[clickedPosition.y][clickedPosition.x] != null &&
                    this.game.gameBoard.getBoard()[clickedPosition.y][clickedPosition.x].pieceType == PieceType.KING){
                if(this.game.gameBoard.getBoard()[clickedPosition.y][clickedPosition.x].getColor() == Color.WHITE) {
                    whiteKingCount--;
                }
            ...
            if(newPiece.pieceType == PieceType.KING){
                if(newPiece.getColor() == Color.WHITE){
                    if(whiteKingCount > 0)
                        return;
                    whiteKingCount++;
                    this.game.gameBoard.kings[0] = newPiece;
                }
                if(newPiece.getColor() == Color.BLACK){
                    if(blackKingCount > 0)
                        return;
                    blackKingCount++;
                    this.game.gameBoard.kings[1] = newPiece;
                }
            }
            this.game.gameBoard.getBoard()[clickedPosition.y][clickedPosition.x] = newPiece;
            this.game.gameBoard.setBoard(clickedPosition.x, clickedPosition.y, newPiece);
    ...
    public void makeMove(Move moveMade, boolean forwarding, boolean humanMove){
        if(revertCount > 0 && !forwarding){
            this.game.gameBoard.deleteFromHistory(revertCount);
            revertCount = 0;
        }
        undecorateLastMove();
        ...
        lastMove.setText(this.game.gameBoard.thisMoveToPgn());
        updatePgnMoveList();
...
//                newWrapper.setOnMouseClicked(new GameboardTileClickHandler(this.game, this));
    ...
    public void createPromotedPiece(Piece piece){
        ...
        lastMove.setText(this.game.gameBoard.thisMoveToPgn());
        movesList.getItems().add(this.game.gameBoard.thisMoveToPgn());
    ...
    public void decorateLastMove(){
        Move move = this.game.gameBoard.getLastMove();
        if(move == null)
            return;
    ...
    public void restoreDecorations(){
        ...
        if (game.gameBoard.isPositionOnBoard(clickedPositionPrevious)){
    ...
    public void updatePgnMoveList(){
        List<String> moves = this.game.gameBoard.getPngMovesList();
    ...
    @FXML
    private void revert(){
        if(this.game.gameBoard.getGUIHistoryCount() - revertCount <= 0)
            return;
    ...
    private void forward(){
        if (revertCount > 0){
            Move move = this.game.gameBoard.getMoveFromGui(revertCount);
    ...
    private void DeletePiece(){
        if(lastWrapperWrapperEditTileSelect != null){
            lastWrapperWrapperEditTileSelect.getStyleClass().removeIf(c -> c.toString().equals("edit-tile-select"));
            if(this.game.gameBoard.getBoard()[lastEditSelectPiece.getPosition().y][lastEditSelectPiece.getPosition().x].pieceType == PieceType.KING){
                if(this.game.gameBoard.getBoard()[lastEditSelectPiece.getPosition().y][lastEditSelectPiece.getPosition().x].getColor() == Color.WHITE)
                    whiteKingCount--;
                else
                    blackKingCount--;
            }
            this.game.gameBoard.getBoard()[lastEditSelectPiece.getPosition().y][lastEditSelectPiece.getPosition().x] = null;
    ...
    private void saveGame(){
        String data = this.game.gameBoard.createPgnString();
        if(this.game.ended){
